...
//  *       select a convolution algorithm. There are two search methods in
//  cuDNN,
...
//  *       time-consuming, and the selected algorithm will be cached for a
//  given
...
//  *       getting algorithms via heuristic search. There are two search
//  methods
//  *       in cuBlasLt, heuristic search and exhaustive search. Exhaustive
//  search
//  *       attempts all cuBlasLt algorithms to select the fastest, which is
//  very
//  *       time-consuming, and the selected algorithm will be cached for a
//  given
//  *       layer specification Once you change the layer specifications
...
//     " epilogue algorithms, default is 0, means disabling exhaustive
//     search.");
...
//     " use Tensor Cores, cuBLAS and cuDNN. cuBLAS uses Tensor Cores to speed
//     up" " GEMM computations(the matrices must be either half precision or
//     single " "precision); cuDNN uses Tensor Cores to speed up both
//     convolutions(the " "input and output must be half precision) and
